...
export function Dashboard() {
  ...
  const COLORS = ['#03245A', '#1691CC', '#8b5cf6', '#f59e0b', '#ef4444'];
  ...
  if (patientsLoading || appointmentsLoading || !dashboardData) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#03245A]"></div>
      </div>
    );
  }

  const statCards = [
    { name: 'Pacientes Totales', value: dashboardData.totalPatients, label: 'Expedientes', icon: Users, color: 'text-[#03245A]', bg: 'bg-blue-50' },
    ...
    { name: 'Ingresos Hoy', value: `$${dashboardData.todayRevenue}`, label: 'MXN', icon: DollarSign, color: 'text-[#1691CC]', bg: 'bg-blue-50' },
    { name: 'Citas Hoy', value: dashboardData.todayAptsCount, label: 'Programadas', icon: Calendar, color: 'text-[#03245A]', bg: 'bg-blue-50' },
  ];
  ...
  return (
    <div className="space-y-8 pb-10">
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <p className="text-slate-500">Bienvenido, Dr. Christian Janis Sandoval.</p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="relative w-full sm:w-80 group">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <Search className="h-4 w-4 text-slate-400 group-focus-within:text-[#03245A] transition-colors" />
            </div>
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl leading-5 bg-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#03245A]/20 focus:border-[#03245A] sm:text-sm transition-all shadow-sm"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <TrendingUp className="w-5 h-5 text-[#1691CC]" />
          </div>
              ...
              <AreaChart data={dashboardData.revenueTrend}>
                <defs>
                  <linearGradient id="colorTotal" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#03245A" stopOpacity={0.1}/>
                    <stop offset="95%" stopColor="#03245A" stopOpacity={0}/>
                  </linearGradient>
                </defs>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/agenda" className="text-xs font-bold text-[#03245A] hover:underline">VER TODAS</Link>
          </div>
          <div className="space-y-4">
              ...
              dashboardData.upcomingToday.map((apt) => (
                <div key={apt.id} className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 border border-slate-100">
                  <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-white border border-slate-200 flex flex-col items-center justify-center">
                    <span className="text-xs font-bold text-[#03245A]">{format(new Date(apt.date), 'HH:mm')}</span>
                  </div>
            ...
            <Link 
              to="/agenda" 
              className="mt-4 w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed border-slate-200 text-slate-500 hover:border-[#03245A] hover:text-[#03245A] transition-all text-sm font-medium"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link to="/patients" className="text-sm font-bold text-[#03245A] hover:underline">VER TODOS</Link>
          </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right hidden sm:block">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      patient.alerts?.length > 0 ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'
                    }`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Link to="/dictation" className="group p-6 bg-gradient-to-br from-[#03245A] to-[#1691CC] rounded-2xl shadow-sm hover:shadow-lg transition-all">
          <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/scales" className="group p-6 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between mb-4">
            <div className="p-3 bg-blue-50 rounded-xl">
              <BarChart3 className="w-6 h-6 text-[#03245A]" />
            </div>
            <ArrowRight className="w-5 h-5 text-slate-300 group-hover:text-[#03245A] transition-colors" />
          </div>
        ...
        <Link to="/patients" className="group p-6 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between mb-4">
            <div className="p-3 bg-blue-50 rounded-xl">
              <Users className="w-6 h-6 text-[#03245A]" />
            </div>
            <ArrowRight className="w-5 h-5 text-slate-300 group-hover:text-[#03245A] transition-colors" />
          </div>
