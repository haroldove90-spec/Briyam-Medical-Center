    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {role === 'assistant' && (
            <div className="bg-[#03245A]/10 text-[#03245A] px-3 py-1 rounded-full text-sm font-medium">
              Modo Asistente Activo
            </div>
          )}
          <button 
            onClick={() => setIsModalOpen(true)}
            className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-[#03245A] hover:bg-[#1691CC] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          >
        ...
        <div className="flex items-center bg-slate-100 dark:bg-slate-200 p-1 rounded-lg self-start md:self-auto">
          <button
            ...
            className={`flex items-center px-3 py-1.5 text-sm font-medium rounded-md transition-all ${
              viewMode === 'calendar'
                ? 'bg-[#03245A] text-white shadow-sm'
            ...
            <div className="lg:col-span-1">
              ...
              <div className="mt-6 bg-white dark:bg-white rounded-xl shadow-sm border border-slate-200 dark:border-slate-200 p-4">
                <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-900 mb-4 flex items-center">
                  <Clock className="h-4 w-4 mr-2 text-[#03245A]" />
                ...
                <div className="space-y-3">
                  {appointments
                    ...
                    .map(apt => (
                      <div key={apt.id} className="flex items-center p-3 rounded-lg border border-slate-100 dark:border-slate-100 hover:bg-blue-50 dark:hover:bg-blue-50/50 transition-colors">
                        <div className="flex-shrink-0 w-12 text-center">
                          <p className="text-xs font-bold text-[#03245A] dark:text-[#03245A]">{format(new Date(apt.date), 'HH:mm')}</p>
                        </div>
                        ...
                        <div className={`h-2 w-2 rounded-full ${
                          ...
                          apt.status === 'completed' ? 'bg-[#03245A]' : 'bg-amber-500'
                        }`} />
                      </div>
                    ))}
              ...
              <div className="bg-white dark:bg-white rounded-xl shadow-sm border border-slate-200 dark:border-slate-200 p-6">
                ...
                <div className="space-y-4">
                  {appointments
                    ...
                    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
                    .map(apt => (
                      <div key={apt.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-200 hover:bg-blue-50/30 hover:shadow-md transition-all gap-4">
                        <div className="flex items-center gap-4">
                          <div className="h-12 w-12 rounded-full bg-[#03245A]/10 flex items-center justify-center text-[#03245A] font-bold">
                            ...
                            {apt.status !== 'completed' && apt.status !== 'cancelled' && (
                              <>
                                <button 
                                  ...
                                  className="p-1.5 text-[#03245A] hover:bg-blue-50 rounded-lg transition-colors"
                                  title="Completar"
                                >
                  ...
                  {appointments.filter(apt => isSameDay(new Date(apt.date), selectedDate)).length === 0 && (
                    <div className="text-center py-12">
                      ...
                      <button 
                        onClick={() => setIsModalOpen(true)}
                        className="mt-4 text-[#03245A] font-medium hover:underline"
                      >
              ...
              <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-200">
                ...
                <tbody className="bg-white dark:bg-white divide-y divide-slate-200 dark:divide-slate-200">
                  {filteredAppointments.map((apt) => (
                    <tr key={apt.id} className="hover:bg-blue-50/50 dark:hover:bg-blue-50/30 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="h-8 w-8 rounded-full bg-[#03245A]/10 flex items-center justify-center text-[#03245A] text-xs font-bold">
                            {apt.patientName.charAt(0)}
                          </div>
                          <div className="ml-3">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end gap-2">
                          {apt.status === 'scheduled' && (
                            <button onClick={() => updateStatus(apt.id, 'confirmed')} className="text-blue-600 hover:text-blue-900">Confirmar</button>
                          )}
                          <button onClick={() => handlePatientClick(apt.patientId, apt.patientName)} className="text-[#03245A] hover:text-[#1691CC]">Ver</button>
                        </div>
                      </td>
                    </tr>
                  ))}
                ...
                <form onSubmit={handleAddAppointment} className="space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-[#03245A] hover:underline"
                    >
                    ...
                    <div className="space-y-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            type="text"
                            required
                            className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-[#03245A] focus:ring-[#03245A] sm:text-sm p-2 border"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <select
                            className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-[#03245A] focus:ring-[#03245A] sm:text-sm p-2 border bg-white"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="relative mt-1 rounded-md shadow-sm">
                        ...
                        <input
                          type="number"
                          className="block w-full rounded-md border-slate-300 pl-7 focus:border-[#03245A] focus:ring-[#03245A] sm:text-sm p-2 border"
                          placeholder="0.00"
                  ...
                  <button
                    ...
                    className="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-[#03245A] hover:bg-[#1691CC] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? (
                      <>
          ...
          <div className="flex min-h-screen items-center justify-center p-4 text-center">
            ...
            <div className="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
              ...
              <div className="bg-slate-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-[#03245A] text-base font-medium text-white hover:bg-[#1691CC] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#03245A] sm:ml-3 sm:w-auto sm:text-sm"
                ...
                <button
                  type="button"
                  className="mt-3 w-full inline-flex justify-center rounded-md border border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#03245A] sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
